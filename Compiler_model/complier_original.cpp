...
class Scanner { // 词法解析器.解析指定文件中所有的字符,划分词类
	FILE * fp;
	char   c;
...
public:
	static const char * TW[], *TD[];   // 关键字,特殊字符数组
	...
	do {
		...
		case IDENT:
			if (isalpha(c) || isdigit(c)) {
				...
				ungetc(c, fp);   // (用c替换c)将输出流中的废弃数据退入流中去
				if ((j = look(buf, TW))) {  // 判断该单词buf是变量名还是关键词
					return Lex((type_of_lex)j, j);
				}
				else {
					j = put(buf);  // 若该变量名已存在, 则返回其在buf数组的位置,若不存在则将该变量名整个放入buf数组再返回其在buf数组的位置
					return Lex(LEX_ID, j);  // j为该变量在buf数组中的位置
				}
			}
			break;
		case NUMB:
...
public:
	vector <Lex> poliz;
	Parser(const char *program) : scan(program) { }  // 同时也初始化了词法分析器
	void  analyze();
};
...
void Parser::D() {
	if (c_type != LEX_ID)
		...
		st_int.push(c_val); // 存入变量名的位置
		gl();
		while (c_type == LEX_COMMA) { // 变量声明语句中的逗号
			gl();
			if (c_type != LEX_ID)
				throw curr_lex;
			else {
...
void Parser::F() {
	if (c_type == LEX_ID) {
		check_id(); // 在全局的变量名数组中检查该变量是否已经声明过
