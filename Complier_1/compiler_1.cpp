...
class Scanner {
	FILE * fp;
	char   c;
	int look(const string buf, const char ** list) {  // 搜索字符串在list中的位置 
		int i = 0;
		while (list[i]) {
			if (buf == list[i])
				return i;
...
public:
	static const char * TW[], *TD[];   // 二维数组 TW---数据类型关键字 和 TD --- 符号关键字 
